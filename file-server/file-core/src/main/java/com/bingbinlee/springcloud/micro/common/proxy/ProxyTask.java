...
public class ProxyTask implements Runnable {
    ...
    private String toUrlServer;//服务器Url
    private String toPortserver;
    ...
    public ProxyTask(Socket socket, String urlServer, String portserver) {
        this.socketIn = socket;
        // 获取配置文件中的代理服务IP和端口
        this.toUrlServer = urlServer;   // 代理服务IP
        this.toPortserver = portserver; // 代理服务端口
    }
    ...
    public void run() {
        ...
        try {
            ...
            header.setMethod(HttpHeader.METHOD_POST);  // 请求方式
            header.setHost(toUrlServer);  // 请求服务器IP
            header.setPort(toPortserver);          // 请求服务器端口
